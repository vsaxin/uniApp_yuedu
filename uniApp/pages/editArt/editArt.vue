                ...
                uni.request({
                    ...
                    success: res => {
                        if(res.data.status == 'ok'){
                            ...
                            // 记录分类信息
                            _self.catiesFromApi = categories; //后台返回的分类信息，与_self.caties不同之处只是少了一个‘请选择’
                            // 获取当前分类的默认值
                            _self.sedCateIndex = art.art_cate;
                            // 对应的查找picker的默认值
                            var cateName = categories[art.art_cate];//后台返回的分类信息[后台的索引]
        ...
        submitNow : function(){
            ...
            for(var i = 0; i < this.artList.length; i++){
                if(this.artList[i].type == 'image'){
                    if(this.artList[i].content.indexOf('192.168.18.') == -1){ //如果服务器不是后台服务器路径。那就需要上传
